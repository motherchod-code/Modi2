...
sock.ev.on("messages.upsert", async (m) => {
  ...
  try {
    ...
    running = true;
    console.log("⚡ ULTRA SMART MODE START");
    ...
    for (let start = 175; start <= 1000; start += batchSize) {
      if (found || failCount >= maxFail) break;

      const controllers = []; // 🔥 cancel control
      ...
      if (failCount >= maxFail) {
        console.log("⛔ STOP → Too many fails");
        break;
      }
