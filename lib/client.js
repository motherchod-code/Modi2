...
sock.ev.on("messages.upsert", async (m) => {
  try {
    ...
    const uniqueKey = jid + messageId;

    // ❗ MAIN FIX (duplicate stop)
    if (reacted.has(uniqueKey)) {
      console.log("⛔ Already reacted, skipping...");
      return;
    }

    reacted.add(uniqueKey);
    ...
    await sock.newsletterReactMessage(jid, messageId, emoji);
