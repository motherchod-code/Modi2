...
let running = false; // 🔥 একসাথে একাধিক loop বন্ধ করবে
...
sock.ev.on("messages.upsert", async (m) => {
  try {
    ...
    if (running) {
      console.log("⛔ Already running loop...");
      return;
    }

    running = true;

    console.log("📢 New Post Detected → Start React Loop");

    const emojis = ["❤️","🔥","😂","😮","😢","👏","😍","🤩"];

    for (let i = 175; i <= 1000; i++) {
      const id = String(i);
