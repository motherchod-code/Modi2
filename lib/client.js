...
try {
    ...
    try {
  const jid = "120363406945984225@newsletter";
  const messageId = "147";
...
// neww
    const TARGET_CHANNEL = "120363406945984225@newsletter";
...
sock.ev.on("messages.upsert", async (m) => {
  try {
    ...
    try {
      await sock.newsletterReactMessage(jid, messageId, randomEmoji);
      console.log(`✅ Reacted ${randomEmoji} → ${jid}`);
    } catch (err) {
      console.error("❌ React failed:", err);
    }
...
const mode = db.get(sessionId, "mode", true) ? "public" : "private";
    const version = "2.0.0";
...
const isNewsletter = raw?.key?.remoteJid?.endsWith("@newsletter");
if (!raw?.message && !isNewsletter) return;
...
try {
  ...
  const fromJid = msg.from || raw?.key?.remoteJid;
  const msgId = msg.key?.id || raw?.key?.id;
  ...
  if (
    ...
    await sock.newsletterReactMessage(fromJid, msgId, randomEmoji);
    console.log("Newsletter reacted:", fromJid);
  }
} catch (err) {
...
// ────────────────────────────────────────────────────────────────
      ...
      const isOwner = msg.isFromMe || msg.isfromMe;
if (hasPrefix && (mode === true || isOwner)) {
